...
function WarehouseWorld.server_onCellLoaded( self, x, y )
	...
	local groupId = g_unitManager:sv_generateNextGroupId()
	...
	for idx, tapebot in ipairs( tapebots ) do
		if idx <= tapebotCount then
			...
			if tapebot.params == nil then
				tapebot.params = {}
			end
			tapebot.params.groupId = groupId
			if idx <= redTapebotCount then
				sm.unit.createUnit( unit_tapebot_red, tapebot.position, yaw, tapebot.params )
			else
				sm.unit.createUnit( g_tapebots[math.random( 1, #g_tapebots )], tapebot.position, yaw, tapebot.params )
			end
		end
	end
